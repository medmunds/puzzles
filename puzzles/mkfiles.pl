...
sub mfval($) {
    ...
    if (grep { $type eq $_ }
	("vc","vcproj","cygwin","borland","lcc","gtk","mpw","nestedvm","osx","wce","gnustep","emcc","html5")) {
	    return 1;
	}
...
if (defined $makefiles{'emcc'}) {
    ...
    ##-- Makefile for building HTML5 puzzles (alt version of emcc, above)
    open OUT, ">$makefiles{'html5'}"; select OUT;
    print
    "# Makefile for $project_name under Emscripten (JavaScript cross compiler).\n".
    "#\n# This file was created by `mkfiles.pl' from the `Recipe' file.\n".
    "# DO NOT EDIT THIS FILE DIRECTLY; edit Recipe or mkfiles.pl instead.\n";
    # gcc command line option is -D not /D
    ($_ = $help) =~ s/=\/D/=-D/gs;
    ...
    &splitline("CFLAGS := -Wall -g -DSTYLUS_BASED -DNARROW_BORDERS " .
	       (join " ", map {"-I$dirpfx$_"} @srcdirs) .
	       " \$(CFLAGS)")."\n".
	"C_EXPORT_LIST := ../js/c_exports.json\n".
	&splitline("LDFLAGS := -O2 --pre-js ../js/pre_js.js --post-js ../js/post_js.js ".
	    "-s TOTAL_MEMORY=0x80000 -s FAST_MEMORY=0x80000 -s TOTAL_STACK=0x8000 ".
    ...
    foreach $p (&prognames("X")) {
      ...
      $objstr =~ s/gtk\.bc/html5\.bc/g; # we need html5.bc rather than gtk.bc
